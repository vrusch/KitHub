...
const APP_VERSION = "v2.5.1-loading-fix";
...
const SettingsModal = ({
  ...
  // Lokální stav pro editaci ID
  const [tempId, setTempId] = useState(currentUid || "");

  // Aktualizace tempId pokud se změní externí ID
  useEffect(() => {
    setTempId(currentUid || "");
  }, [currentUid]);
  ...
  const handleLoadId = () => {
    if (tempId && tempId.trim().length > 0) {
      onSetUid(tempId.trim());
    }
  };
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex justify-between">
              <span>ID Deníku (pro synchronizaci)</span>
              {tempId !== currentUid && (
            ...
            <div className="flex gap-2">
              <input
                className={`flex-1 bg-slate-950 border ${tempId !== currentUid ? "border-orange-500 text-white" : "border-slate-800 text-slate-300"} rounded-lg p-3 text-sm font-mono break-all outline-none focus:border-blue-500 transition-colors`}
                value={tempId}
                onChange={(e) => setTempId(e.target.value)}
                placeholder="Vložte ID deníku..."
              />

              {tempId !== currentUid && (
                <button
                  onClick={handleLoadId}
                  className="bg-blue-600 hover:bg-blue-500 text-white p-3 rounded-lg shadow-lg animate-in slide-in-from-right-2"
                  title="Načíst data pro toto ID"
                >
            ...
            <p className="text-[10px] text-slate-500">
              Zde můžete ručně zadat ID z jiného zařízení pro načtení dat.
            </p>
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-slate-800">
            ...
            <div className="text-[10px] bg-slate-950 p-2 rounded text-slate-400 font-mono space-y-1">
              <div className="flex justify-between">
                ...
                >
                  {firebaseConfig.apiKey ? "NALEZEN" : "CHYBÍ"}
                </span>
              </div>
              {initError && (
